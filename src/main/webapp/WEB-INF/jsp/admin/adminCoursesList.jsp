    ...
    <div class="btn-group dropright">
        ...
        <div class="dropdown-menu">
            <c:forEach var="topic" items="${allTopic}">
                <a class="dropdown-item" href="/adminCourseByTopicProgress?topic_id=${topic.id}"><c:out
                        value="${topic.name}"/></a>
            </c:forEach>
        </div>
    </div>
    ...
    <c:otherwise><br/>
        <c:forEach var="course" items="${progress}">
            <tr>
                ...
                <td>
                    <form id="status1" action="/changeCourseStatus?status=finished&course_id=${course.id}"
                          method="post">
